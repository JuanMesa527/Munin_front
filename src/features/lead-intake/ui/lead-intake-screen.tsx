...
export interface LeadIntakeScreenProps {
  ...
  /**
   * Se dispara UNA vez cuando el perfilamiento termina en carril `no_viable`,
   * con el `leadId` para que `app/` pase el control a F2.2 (camino de
   * nutrición). Igual que `onViable`: F1 no conoce a F2.2, solo avisa.
   */
  onNoViable?: (leadId: string) => void;
}

export function LeadIntakeScreen({
  onViable,
  onNoViable,
}: LeadIntakeScreenProps = {}): ReactElement {
  const {
    phase,
    turn,
    messages,
  ...
  // Handoff F1 -> F2.2: al cerrar en `no_viable`, avisa a `app/` con el leadId.
  useEffect(() => {
    if (phase === 'completado-no-viable' && turn !== null) {
      onNoViable?.(turn.profile.id);
    }
  }, [phase, turn, onNoViable]);
